...
// Contrôleurs pour les priorités
...
exports.updatePriorite = async (req, res) => {
    try {
        const priorite = await Priorite.findByIdAndUpdate(
            req.params.id,
...
exports.deletePriorite = async (req, res) => {
    try {
        const priorite = await Priorite.findByIdAndDelete(req.params.id);
        if (!priorite) {
            return res.status(404).json({ message: 'Priorité non trouvée' });
        }
...
exports.addDocumentIndividually = async (req, res) => {
    try {
        ...
        preuve.documents.push(...newDocuments);
        preuve.updatedAt = new Date();
        await preuve.save();

        res.status(200).json(preuve);
    } catch (error) {
        res.status(400).json({ message: error.message });
    }
};
    ...
    try {
        ...
        for (const doc of preuve.documents) {
            try {
                ...
                console.error(`Erreur lors de la suppression du fichier ${doc.path}:`, err);
            }
        }
...
exports.updatePreuves = async (req, res) => {
    try {
        const preuve = await Preuve.findByIdAndUpdate(
            req.params.idPreuve,
            { 
                ...req.body,
                updatedAt: new Date()
...
exports.deleteDocumentPreuve = async (req, res) => {
    try {
        ...
        try {
            ...
            console.error(`Erreur lors de la suppression du fichier ${document.path}:`, err);
        }
...
// Contrôleurs pour les types de priorités
...
exports.getByIdTypePriorite = async (req, res) => {
    try {
        ...
        if (!type) {
            return res.status(404).json({ message: 'Type de priorité non trouvé' });
        }
        res.json(type);
    } catch (error) {
        res.status(500).json({ message: error.message });
...
exports.updateTypePriorite = async (req, res) => {
    try {
        const type = await TypePriorite.findByIdAndUpdate(
            req.params.id,
            { ...req.body, updatedAt: new Date() },
            { new: true }
        );
...
exports.deleteTypePriorite = async (req, res) => {
    try {
        const type = await TypePriorite.findByIdAndDelete(req.params.id);
        if (!type) {
            return res.status(404).json({ message: 'Type de priorité non trouvé' });
        }
